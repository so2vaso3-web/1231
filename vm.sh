...
SSH_PORT=24
DISK_SIZE=100G
GUI_MODE=false  # Set to true for GUI (window display), false for console only
...
if [ "$GUI_MODE" = true ]; then
    echo "[INFO] GUI Mode: VM window will open"
    echo "[INFO] After login, install desktop: apt update && apt install -y ubuntu-desktop-minimal"
    ...
    echo "[INFO] Console Mode: No GUI (text only)"
    exec qemu-system-x86_64 \
        -enable-kvm \
        -m "$MEMORY" \
        -smp "$CPUS" \
